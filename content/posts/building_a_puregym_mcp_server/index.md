...
I have spent the last few weeks building something that's either a neat developer tool, or an unhinged
side-project, or maybe both: an [MCP server](https://modelcontextprotocol.io/docs/getting-started/intro)
exposing PureGym's functionality.
...
sponsored by PureGym. PureGym is a registered trademark of Pure Gym Limited.
...
It's not even trying to pronounce _rødgrød med fløde_ without choking. It's their **obsession** around
scheduling.
...
So the game becomes: book early enough to get a spot, then remember what you booked, and cancel in time if
plans change so you do not get charged a fee. I am definitely not built for this. Whenever I tried doing so I
either booked too late or forgot I had booked at all.

So, I did the obvious reasonable totally sane thing: built a Telegram bot that books for me and sends me
reminders about the classes I booked. And once I had the API layer somewhat figured out, I gave in to the
LLM-hype and decided to expose that to an AI client via MCP.
...
not available on the web, like live center occupancy and expected occupancy by hour throughout the week.

And so, the hardest part of this project was **not** writing the MCP server, or even figuring out the auth
flows. It was getting reliable visibility into what the mobile app was actually doing.
...
After a bit of reading, I found that some apps accept certificates that are installed in the system, which you
can do if you have a rooted phone! So, I took the old phone I had from
...
[this article](https://docs.mitmproxy.org/stable/howto/install-system-trusted-ca-android/), to emulate a
rooted Android device. That worked, I could install the mitmproxy certificates in the system and it worked
fine. But PureGym **still** wouldn't budge. Allow me to introduce the concept of **certificate pinning**,
where the application or device has a preconfigured list of public keys or certificates that it explicitly
trusts, specifically to prevent MITM attacks.
...
That works great for tools like OpenCode, Claude Code, VS Code, Codex, and the list keeps growing. You add a
small config block and off you go, especially if you already use [uv](https://docs.astral.sh/uv/).
...
clients consume it. That is great right up until you remember that your hosted PureGym MCP is now reachable by
more than just you.
...
you** if you don't provide a token too.
